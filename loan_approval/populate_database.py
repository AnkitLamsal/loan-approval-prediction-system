...
class Loader:
    ...
    def __init__(self):
        logging.basicConfig(filename='app.log', filemode='w', format='%(asctime)s %(name)s - %(levelname)s - %(message)s', level=logging.DEBUG,  datefmt='%d-%b-%y %H:%M:%S')
        ...
        if num_employees == num_applicants:
            for i in range(num_employees):
                ...
                for j in range(0,250):
                    print((j+1)*(i))
                    # pass
                    logging.info("Data Instance: {0}".format((j+1)*(i)))
                    self.populate_data_single_instance((j+1)*i,employee=employee, applicant=applicant)
    ...
    def populate_loan(self,data,applicant, employee):
        ...
        logging.info("""PERSON AGE: {0},HOME OWNERSHIP: {1}, EMP LENGTH: {2}, AMOUNT: {3}, INCOME: {4}, INTENT: {5}""".format(age,home_ownership,
                                                                     emp_length,amount,
                                                                     income,intent))
        # print(applicant,employee,age,home_ownership,int(emp_length),amount,income,intent)
        # print("Loan Object Created.")
        try:
